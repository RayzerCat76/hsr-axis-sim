...
immutable `RuntimeEvent`; it does not hook production dispatch and cannot send
an envelope back into the simulator.
...
`payload.adapter`. Values are not rounded or converted to trace numeric data.
...
Production `BattleState.emit_event`, trigger dispatch, simulator events,
timeline behavior, formulas, lifecycle behavior, and the existing LIFO
extra-turn stack remain unchanged. A future export layer may manually consume
these envelopes at this read-only boundary without turning the adapter into a
production hook.
