...
def _build_action(case: RuntimeActionSessionRegressionCase, index: int) -> Action:
    ...
    if isinstance(setup, RuntimeActionSessionRegressionEnergyGainSetup) and setup.action_index == index:
        ...
        isinstance(setup, RuntimeActionSessionRegressionChangeSpeedSetup)
        and setup.action_index == index
    ):
        effects = [ChangeSpeed(target_ids=[setup.target_id], new_speed=setup.new_speed)]
    elif (
        isinstance(setup, RuntimeActionSessionRegressionImmediateActionSetup)
        and setup.action_index == index
    ):
