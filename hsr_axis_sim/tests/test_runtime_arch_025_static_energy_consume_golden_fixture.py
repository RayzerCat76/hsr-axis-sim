...
EXPECTED_PATH = (
    ...
    / "arch_025_reviewed_energy_consume_expected.json"
)
ARCH_017_PATH = (
    PACKAGE_ROOT
    / "data"
...
EXPECTED_SHA256 = "7d61528687a5a2f499249e0f914f6f2f50975c7c153165eddd5e116f3ed19a75"
EXPECTED_SIZE_BYTES = 2750
...
FIXTURE_ID = "arch-025-reviewed-static-energy-consume"
STREAM_ID = "arch-025-reviewed-resource"
ACTION_ID = "reviewed-energy-consume"
ACTOR_ID = "consume-actor"
...
def _runtime_inputs(*, consume_amount: float = 30):
    steps = (
        ExplicitActionCaptureStep(
            Action(
                ACTION_ID,
                ACTION_ID,
                ACTOR_ID,
                effects=[ConsumeEnergy(target_ids=[TARGET_ID], amount=consume_amount)],
                ends_turn=False,
            )
        ),
    )
    ...
    stitch_config = CapturedTraceStitchConfig(
        TraceExportConfig(
            "arch-025-resource-runtime-actual",
            TraceSequencePolicy.CONTIGUOUS,
            EmptyTracePolicy.REJECT,
...
def test_static_energy_consume_fixture_has_exact_reviewed_bytes_digest_and_schema():
    ...
    assert document.trace_id == "arch-025-reviewed-static-expected"
...
def test_changed_consume_amount_reports_resource_divergence_and_signed_values():
    ...
    assert expected_resource["requested_delta"] == -30
    assert actual_resource["requested_delta"] == -25
    assert expected_resource["applied_delta"] == -30
    assert actual_resource["applied_delta"] == -25
    assert expected_resource["cap"] == actual_resource["cap"] == 100
...
def test_prior_static_fixtures_and_arch025_prefix_remain_accepted():
