...
def _manifest(version: str, setup_marker=...):
    ...
    return {
        ...
        "version": version,
        "manifest_id": "arch-026-version-probe",
        "cases": [case],
    }
...
def _parse(data):
    return runtime_action_session_regression_manifest_from_dict(
        data,
        ROOT / "arch_026_synthetic_manifest.json",
    )


def test_manifest_versions_preserve_v1_0_through_v1_5():
...
def test_v1_3_requires_setup_and_accepts_exact_four_closed_kinds():
    ...
    assert energy_consume.cases[0].setup == RuntimeActionSessionRegressionEnergyConsumeSetup(
        target_id="consume-target",
        target_name="consume-target",
        team="ally",
        base_speed=100,
...
def test_energy_consume_setup_is_frozen():
    setup = RuntimeActionSessionRegressionEnergyConsumeSetup(
        target_id="consume-target",
        target_name="consume-target",
        team="ally",
        base_speed=100,
...
def test_current_manifest_preserves_arch026_first_four_reviewed_cases_in_order():
    ...
    assert [action.action_id for action in fourth.actions] == [
        "reviewed-energy-consume"
    ]
    assert [action.ends_turn for action in fourth.actions] == [False]
    assert fourth.setup == RuntimeActionSessionRegressionEnergyConsumeSetup(
        target_id="consume-target",
        target_name="consume-target",
        team="ally",
        base_speed=100,
...
def test_energy_consume_harness_change_surfaces_reviewed_after_divergence():
    ...
    report = run_runtime_action_session_regression(
        RuntimeActionSessionRegressionManifest(
            manifest_id="arch-026-controlled-consume-mismatch",
            path=ROOT / "arch_026_controlled_consume_mismatch.json",
            cases=(changed,),
        )
    )
