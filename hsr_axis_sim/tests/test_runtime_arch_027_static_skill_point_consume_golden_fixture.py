...
from hsr_axis_sim.runtime_action_sessions import ExplicitActionCaptureStep, MultiActionCaptureSessionConfig
from hsr_axis_sim.runtime_adapters import AmbiguousLegacyEventPolicy, LegacyEventAdapterConfig, UnknownLegacyEventPolicy
...
from hsr_axis_sim.runtime_exports import EmptyTracePolicy, TraceExportConfig, TraceSequencePolicy
from hsr_axis_sim.runtime_golden_replays import GoldenReplayValidationConfig
from hsr_axis_sim.runtime_loaders import RuntimeTraceLoadConfig, TraceCanonicalForm, TraceCanonicalFormPolicy, TraceDigestPolicy, TraceDigestStatus, load_runtime_trace_bytes
...
EXPECTED_PATH = PACKAGE_ROOT / "data" / "runtime_golden_fixtures" / "arch_027_reviewed_skill_point_consume_expected.json"
...
RUNTIME_REGRESSION_MANIFEST = PACKAGE_ROOT / "data" / "runtime_action_session_regression_manifest.json"
...
def _runtime_inputs(*, consume_amount: int = 2):
    steps = (ExplicitActionCaptureStep(Action(ACTION_ID, ACTION_ID, ACTOR_ID, effects=[ConsumeSkillPoint(amount=consume_amount)], ends_turn=False)),)
    adapter = LegacyEventAdapterConfig(STREAM_ID, UnknownLegacyEventPolicy.REJECT, AmbiguousLegacyEventPolicy.REJECT)
    session_config = MultiActionCaptureSessionConfig(PendingEventCaptureCursor(0, 0), adapter, (TraceExportConfig("arch-027-resource-segment-0", TraceSequencePolicy.CONTIGUOUS, EmptyTracePolicy.REJECT, {"fixture_id": FIXTURE_ID, "segment": 0}),), False)
    stitch_config = CapturedTraceStitchConfig(TraceExportConfig("arch-027-resource-runtime-actual", TraceSequencePolicy.CONTIGUOUS, EmptyTracePolicy.REJECT, {"fixture_id": FIXTURE_ID, "source": "production-action-session"}), False)
    golden_config = GoldenReplayValidationConfig(FIXTURE_ID, EXPECTED_SHA256, TraceCanonicalFormPolicy.COMPACT_ONLY, 100_000)
...
def test_static_skill_point_consume_fixture_has_exact_reviewed_bytes_digest_and_schema():
    ...
    loaded = load_runtime_trace_bytes(expected, config=RuntimeTraceLoadConfig(TraceCanonicalFormPolicy.COMPACT_ONLY, TraceDigestPolicy.REQUIRE_MATCH, EXPECTED_SHA256, 100_000))
    ...
    assert [record.event.event_type for record in document.records] == [RuntimeEventType.ACTION_START, RuntimeEventType.SKILL_POINTS_CHANGED, RuntimeEventType.ACTION_END]
    ...
    assert dict(resource.payload["resource_change"]) == {"resource_kind":"SKILL_POINTS","scope":"TEAM","before":4,"after":2,"requested_delta":-2,"applied_delta":-2,"cap":5,"unit_id":None}
    assert dict(resource.payload["legacy_data"]) == {"actor_id":ACTOR_ID,"action_id":ACTION_ID,"resource_kind":"SKILL_POINTS","scope":"TEAM","before":4,"after":2,"requested_delta":-2,"applied_delta":-2,"cap":5,"unit_id":None}


def test_arch_016_production_successful_skill_point_consume_matches_static_fixture():
    expected = _expected_bytes(); steps, session_config, stitch_config, golden_config = _runtime_inputs(); state = _state()
    ...
    assert [event.type for event in state.pending_events] == ["action_started", "skill_points_changed", "action_finished"]
...
def test_changed_consume_amount_reports_resource_divergence_and_signed_values():
    expected = _expected_bytes(); steps, session_config, stitch_config, golden_config = _runtime_inputs(consume_amount=1); state = _state()
    result = run_action_session_validation(state, steps, session_config=session_config, stitch_config=stitch_config, expected_payload_bytes=expected, golden_config=golden_config)
...
def test_skill_point_consume_fixture_is_promoted_only_into_runtime_regression_manifest():
    legacy = LEGACY_REGRESSION_MANIFEST.read_text(encoding="utf-8"); runtime = RUNTIME_REGRESSION_MANIFEST.read_text(encoding="utf-8")
...
def test_arch_027_test_source_has_no_runtime_expected_generation_path():
    tree = ast.parse(Path(__file__).read_text()); called_names = set()
    for node in ast.walk(tree):
        if isinstance(node, ast.Call):
            if isinstance(node.func, ast.Name): called_names.add(node.func.id)
            elif isinstance(node.func, ast.Attribute): called_names.add(node.func.attr)
...
def test_prior_static_fixtures_and_arch027_prefix_remain_accepted():
    ...
    assert [result.case_id for result in report.results[:5]] == ["arch-017-reviewed-static-action-session","arch-021-reviewed-static-clamped-energy","arch-023-reviewed-static-clamped-skill-point","arch-025-reviewed-static-energy-consume","arch-027-reviewed-static-skill-point-consume"]


def test_legacy_regression_and_trace_evidence_remain_unchanged():
    legacy=load_regression_manifest(LEGACY_REGRESSION_MANIFEST); complete=run_regression(manifest=legacy); trace=run_regression(manifest=legacy,only="trace_evidence")
...
def test_production_lifo_compatibility_remains_unchanged():
    units=[Unit(name,name,"ally",100) for name in ("first","second","third")]; state=BattleState(units=units,extra_turn_stack=["first","second","third"])
