...
def _action_delay_setup(*, percent=0.25, initial_av=30):
    return {
        "kind": "ACTION_DELAY",
...
def test_v1_5_explicitly_rejects_action_delay_as_v1_6_syntax():
    with pytest.raises(ValueError, match="requires manifest version '1.6'"):
        _parse(
            _manifest(
                RUNTIME_ACTION_SESSION_REGRESSION_VERSION_1_5,
                _action_delay_setup(),
...
def test_v1_5_action_advance_remains_valid_after_v1_6_addition():
    parsed = _parse(
        _manifest(
            RUNTIME_ACTION_SESSION_REGRESSION_VERSION_1_5,
...
def test_v1_6_accepts_seventh_closed_action_delay_kind():
    parsed = _parse(_manifest(RUNTIME_ACTION_SESSION_REGRESSION_VERSION, _action_delay_setup()))
...
def test_action_delay_setup_is_frozen():
    setup = RuntimeActionSessionRegressionActionDelaySetup(
        target_id="delay-actor",
        target_name="Delay Actor",
        team="ally",
        base_speed=100,
...
def test_action_delay_fields_are_exact_and_generic_kind_is_rejected():
    extra = _action_delay_setup()
    extra["mode"] = "delay"
    with pytest.raises(ValueError, match="unsupported field"):
        _parse(_manifest("1.6", extra))

    missing = _action_delay_setup()
    missing.pop("percent")
    with pytest.raises(ValueError, match="missing required field"):
        _parse(_manifest("1.6", missing))

    generic = _action_delay_setup()
    generic["kind"] = "GENERIC_AXIS_EFFECT"
    with pytest.raises(ValueError, match="kind"):
        _parse(_manifest("1.6", generic))
...
def test_action_delay_percent_is_not_silently_restricted_positive(percent):
    parsed = _parse(_manifest("1.6", _action_delay_setup(percent=percent)))
...
def test_action_delay_initial_av_is_finite_but_not_newly_range_restricted(initial_av):
    parsed = _parse(
        _manifest("1.6", _action_delay_setup(initial_av=initial_av))
    )
...
def test_action_delay_harness_change_surfaces_structured_after_av_divergence():
    ...
    changed = replace(case, setup=replace(case.setup, percent=0.20))
...
def test_action_delay_regression_harness_is_closed_and_explicitly_targeted():
    ...
    assert "ACTION_DELAY" in manifest_source
