...
from hsr_axis_sim.sim.effects import ChangeSpeed
...
EXPECTED_PATH = FIXTURE_DIR / "arch_038_reviewed_change_speed_expected.json"
...
def _state() -> BattleState:
    return BattleState(
        [Unit(ACTOR_ID, "Speed Actor", "ally", 100, current_av=80)]
    )


def _runtime_inputs(*, new_speed: float = 200):
    steps = (
        ExplicitActionCaptureStep(
            Action(
                ACTION_ID,
                ACTION_ID,
                ACTOR_ID,
                effects=[ChangeSpeed(new_speed=new_speed)],
                ends_turn=False,
            )
        ),
    )
...
def test_static_change_speed_fixture_has_exact_reviewed_bytes_digest_and_schema():
    ...
    assert document.trace_id == "arch-038-reviewed-static-expected"
    ...
    assert [record.event.event_id for record in document.records] == [
        "legacy:arch-038-reviewed-axis:0",
        "legacy:arch-038-reviewed-axis:1",
        "legacy:arch-038-reviewed-axis:2",
    ]
    ...
    changed = document.records[1].event
    exact_observation = {
        "target_id": ACTOR_ID,
        "before_speed": 100,
        "after_speed": 200,
    ...
    assert dict(changed.payload["speed_change"]) == exact_observation
    assert dict(changed.payload["legacy_data"]) == {
        "actor_id": ACTOR_ID,
        "action_id": ACTION_ID,
        **exact_observation,
    }
    assert changed.payload["adapter"]["legacy_event_type"] == "speed_changed"
    assert changed.payload["adapter"]["mechanic_id"] == "LEGACY_EVENT.SPEED_CHANGED"
    assert changed.payload["adapter"]["mapping_status"] == "BOUND"


def test_arch_016_production_change_speed_matches_static_fixture():
    expected = _expected_bytes()
    ...
    assert actor.speed == 200
    assert actor.current_av == pytest.approx(40.0, abs=1e-12)
...
def test_changed_requested_speed_reports_existing_first_structured_divergence():
    expected = _expected_bytes()
    steps, session_config, stitch_config, golden_config = _runtime_inputs(new_speed=160)
    state = _state()
    ...
    actor = state.get_unit(ACTOR_ID)
    assert actor.speed == 160
    assert actor.current_av == pytest.approx(50.0, abs=1e-12)
    golden = result.validation_result.validation_result.validation_result
    ...
    assert divergence.first_field_difference.path == "/event/payload/legacy_data/after_av"
    assert divergence.first_field_difference.expected_value == 40.0
    assert divergence.first_field_difference.actual_value == 50.0

    expected_speed = golden.comparison.records[1].expected_record.event.payload[
        "speed_change"
    ]
    actual_speed = golden.comparison.records[1].actual_record.event.payload[
        "speed_change"
    ]
    assert expected_speed["target_id"] == actual_speed["target_id"] == ACTOR_ID
    assert expected_speed["before_speed"] == actual_speed["before_speed"] == 100
    assert expected_speed["before_av"] == actual_speed["before_av"] == 80
    assert expected_speed["after_speed"] == 200
    assert actual_speed["after_speed"] == 160
    assert expected_speed["after_av"] == 40.0
    assert actual_speed["after_av"] == 50.0


def test_arch_038_fixture_is_promoted_only_into_runtime_regression_manifest():
...
def test_arch_038_test_source_has_no_runtime_expected_generation_path():
