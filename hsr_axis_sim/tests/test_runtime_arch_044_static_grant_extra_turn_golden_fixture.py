...
from hsr_axis_sim.sim.effects import GrantExtraTurn
...
EXPECTED_PATH = FIXTURE_DIR / "arch_044_reviewed_grant_extra_turn_expected.json"
...
EXPECTED_SHA256 = "57eefb521cb5cf1840e49c36e5c9c85a08281a7014c23ece0e3d5df1e6dfefdd"
EXPECTED_SIZE_BYTES = 2658
...
PREQUEUED_ID = "prequeued-extra-turn"
...
def _state(*, preload_extra_turn: bool = False) -> BattleState:
    units = [
        Unit(ACTOR_ID, "Extra Turn Actor", "ally", 100, current_av=20),
        Unit(TARGET_ID, "Extra Turn Target", "ally", 100, current_av=80),
    ]
    stack = []
    if preload_extra_turn:
        units.append(Unit(PREQUEUED_ID, "Prequeued Extra Turn", "ally", 100, current_av=70))
        stack.append(PREQUEUED_ID)
    return BattleState(units, extra_turn_stack=stack)
    ...
    steps = (
        ExplicitActionCaptureStep(
            Action(
                ...
                ACTION_ID,
                ACTOR_ID,
                effects=[GrantExtraTurn(target_ids=[TARGET_ID])],
                ends_turn=False,
            )
        ),
    )
...
def test_static_grant_extra_turn_fixture_has_exact_reviewed_bytes_digest_and_schema():
    ...
    assert document.trace_id == "arch-044-reviewed-static-expected"
    ...
    queued = document.records[1].event
    exact_observation = {
        "target_id": TARGET_ID,
        "stack_depth_before": 0,
        "stack_depth_after": 1,
    }
    assert dict(queued.payload["extra_turn_queue"]) == exact_observation
    assert dict(queued.payload["legacy_data"]) == {
        "actor_id": ACTOR_ID,
        "action_id": ACTION_ID,
        **exact_observation,
    }
    assert queued.payload["adapter"]["legacy_event_type"] == "extra_turn_queued"
    assert queued.payload["adapter"]["mechanic_id"] == "LEGACY_EVENT.EXTRA_TURN_QUEUED"
    assert queued.payload["adapter"]["mapping_status"] == "BOUND"


def test_arch_016_production_grant_extra_turn_matches_static_fixture():
    expected = _expected_bytes()
    ...
    assert state.extra_turn_stack == [TARGET_ID]
...
def test_preloaded_stack_reports_exact_first_extra_turn_queue_depth_divergence():
    expected = _expected_bytes()
    steps, session_config, stitch_config, golden_config = _runtime_inputs()
    state = _state(preload_extra_turn=True)
    ...
    assert state.get_unit(TARGET_ID).current_av == 80
    assert state.extra_turn_stack == [PREQUEUED_ID, TARGET_ID]
    golden = result.validation_result.validation_result.validation_result
    ...
    assert (
        ...
        == "/event/payload/extra_turn_queue/stack_depth_after"
    )
    assert divergence.first_field_difference.expected_value == 1
    assert divergence.first_field_difference.actual_value == 2

    expected_queue = golden.comparison.records[1].expected_record.event.payload[
        "extra_turn_queue"
    ]
    actual_queue = golden.comparison.records[1].actual_record.event.payload[
        "extra_turn_queue"
    ]
    assert expected_queue["target_id"] == actual_queue["target_id"] == TARGET_ID
    assert expected_queue["stack_depth_before"] == 0
    assert actual_queue["stack_depth_before"] == 1
    assert expected_queue["stack_depth_after"] == 1
    assert actual_queue["stack_depth_after"] == 2
    ...
    assert expected_legacy["stack_depth_before"] == 0
    assert actual_legacy["stack_depth_before"] == 1
    assert expected_legacy["stack_depth_after"] == 1
    assert actual_legacy["stack_depth_after"] == 2


def test_arch_044_fixture_is_not_promoted_into_any_regression_manifest():
...
def test_arch_044_test_source_has_no_runtime_expected_generation_path():
